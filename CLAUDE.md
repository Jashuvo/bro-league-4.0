...
Required env vars (see `.env.example` for the full list with comments) include `VITE_FPL_LEAGUE_ID`, `VITE_LEAGUE_NAME`, `VITE_ENTRY_FEE`, `VITE_TOTAL_PRIZE_POOL`, `VITE_SEASON` (bump by hand every rollover, alongside `leagueConfig.season` in `src/data/leagueData.js` — used to tag rows written into `season_archive`), `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` (the cache + season archive + push subscriptions), `CRON_SECRET` (Vercel signs the cron requests with this automatically once it's set — without it, `warm-cache.js` accepts unsigned requests), `EXCLUSION_PIN` (gates writes to the shared exclusion list), and the web-push trio `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` (generate with `npx web-push generate-vapid-keys`; without them the matchday-notifications feature hides itself in the UI and the cron skips sending). `VITE_FPL_LEAGUE_ID` has no fallback on purpose — unset it and the app fails loudly instead of silently showing a stale league.

The Vercel project is linked to this GitHub repo (`productionBranch: master`) and auto-deploys on every push — confirmed via the Vercel API (`GET /v6/deployments`), a deployment with `source: "git"` and `meta.githubCommitSha` matching the just-pushed commit appeared within well under a minute of `git push`. `git push origin master` alone is enough; a manual `vercel deploy --prod` on top of that is redundant, not required (it was run that way for most of this project's early history simply because nobody had confirmed the auto-deploy was actually live — it was, the whole time).
