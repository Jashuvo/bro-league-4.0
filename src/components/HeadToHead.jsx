...
const HeadToHead = ({ standings = [], gameweekTable = [], gameweekInfo = {}, loading = false, embedded = false }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      {/* Table + fixtures side by side on wide screens (this is the layout
          the old two-manager "Compare" tool used to occupy — one panel each
          side, nothing to scroll past) and stacked, table-first, on mobile
          where there's no room for two columns. `items-start` keeps the
          shorter column from stretching to match the taller one. */}
      <div className="lg:grid lg:grid-cols-[3fr_2fr] lg:items-start lg:gap-6 space-y-6 lg:space-y-0">
